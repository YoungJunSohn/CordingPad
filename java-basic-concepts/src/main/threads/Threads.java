...
public class Threads extends Thread implements Runnable{
    ...
    * */
    ...
    첫 번째로, 다음과 같이 Runnable을 매개 값으로 갖는 생성자를 호출하는 방법이 있다.
    ...
    * */
    ...
    /*스레드의 생성2
    * 두 번째 방법은 Thread 클래스를 상속받는 것이다.
    * Thread 클래스는 내부에 run() 메서드를 갖고 있는데 이를 통해 실행할 서브 스레드를 만들수 있다.
    * Thread 클래스를 상속받아 만든 run()은 Thread 클래스의 run() 메서드를 호출한다는 점에서 Runnable 인터페이스의 run()과 차이점이 있다.
    * */
